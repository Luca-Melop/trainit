...
def update_hint(method, previous_hint, updates, count_inc, prev_params, m_t, beta_3=0.9):
    if method == 0:
        ...
        #hint = prev_params
        hint = jtu.tree_map(lambda ph: ( ph / utils.tree_l2_norm(prev_params) ), prev_params)

    elif method == 20:
        hint = jtu.tree_map(lambda ph: prev_params, prev_params)
    else:
...
def oftrl( #with cheating hint (no adaptive)
    ...
    def update_fn(updates, state, params, hint):
        ...
        scale = jax.lax.cond( #with debiasing
            is_zero_hint,
        ...
        #scale = jtu.tree_map(lambda ws, ss: (eta * ws) / (2 * jnp.sqrt(ss + epsilon)), new_weighted_sum, new_sum_squared)
...
def adaptive_oftrl( #with cheating hint (with adaptive)
    lr: optax.ScalarOrSchedule,
    beta_1: float = 0.9,
    beta_2: float = 0.999,
    beta_3: float =0.9,
    ...
    def update_fn(updates, state, params, hint):
       ...
       # if jtu.tree_all(jtu.tree_map(lambda h: jnp.all(h == 0), hint)):
        ...
        hint = update_hint(hint_method, previous_hint, updates, count_inc, beta_3)
        ...
        new_state = OFTRL2State(
            ...
            logging=logger(**{
                ...
                "hint/cos(grad, prev_hint)": utils.tree_cosine_similarity(previous_hint, updates),
                "hint/cos(hint, prev_hint)": utils.tree_cosine_similarity(previous_hint, hint),
                "hint/distance(hint, prev_hint)": utils.tree_cosine_similarity(previous_hint, hint),
            })
        )
