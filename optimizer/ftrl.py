...
def ftrl(
    ...
    beta_1: float = 0.9,
    beta_2: float = 0.999, #beta_2 = beta_1^2
    epsilon: float = 1e-8,
) -> OnlineLearner:
    ...
    def update_fn(updates, state, params, hint):
        ...
        scale = jtu.tree_map(lambda ws, ss: (eta * (ws / (1 - beta_1 ** count_inc))) / (jnp.sqrt(ss / (1 - beta_2 ** count_inc) + epsilon)), new_weighted_sum, new_sum_squared)
